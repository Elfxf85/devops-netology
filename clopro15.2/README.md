...
resource "yandex_storage_bucket" "mii" {
...
Тукущую дату в названии будет добавлять локальная переменная current_timestamp в формате "день-месяц-год":

```
locals {
    current_timestamp = timestamp()
    formatted_date = formatdate("DD-MM-YYYY", local.current_timestamp)
    bucket_name = "mii-${local.formatted_date}"
}
```
...
Cтартовая веб-страницу будет создана спомощью параметра user_data в разделе metadata:

```
    user-data  = <<EOF
#!/bin/bash
cd /var/www/html
echo '<html><head><title>Picture from S3</title></head> <body><h1>Hello from S3</h1><img src="http://${yandex_storage_bucket.mii.bucket_domain_name}/picture1.jpg"/></body></html>' > index.html
EOF
```
...
Балансировщик нагрузки будет проверять доступность порта 80 и путь "/" при обращении к целевой группе виртуальных машин. Проверка будет выполняться с интервалом 2 секунды, с таймаутом 1 секунда. Пороговые значения для определения состояния сервера будут следующими: 2 неудачные проверки для перевода сервера LAMP в недоступное состояние и 4 успешных проверок для возврата в доступное состояние.
...
![img_6](IMG/img_6.png)
 ...
 Балансировщик нагрузки работает исправно.
...
Для проверки состояния добавляю Healthcheck:
